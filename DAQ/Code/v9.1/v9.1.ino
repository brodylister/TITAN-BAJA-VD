...
Description:
- Version currently works for:
  - IMU
  - SD
  - RTC (including updating time if power was lost)
- Signs of Leon's vibe coding removed
...
void setup() {
  ...
  // [NEW] >>> rtcOk reads false at compile-time. Just check if rtc.lostPower() to adjust clock.
  if (rtc.lostPower()) {
